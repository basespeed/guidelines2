                    ...
                    @foreach($data_color as $color)
                        @if($color->layout_color == 7 && $color->color_menu == $id_menu_check || $color->layout_color == 7 && $color->color_menu_child == $id_menu_check)
                            ...
                            <div class="list_color list_color{{$count1}}">
                                <div class="edit_color"><input type="text" data-id="{{$color->id_color}}" data-menu="@php if(isset($id_menu_check)){echo $id_menu_check;} @endphp" class="cp-full{{$count1}} section7_color{{$count1}}" value="{{$color->hex}}" name="section7_color1" placeholder="Nhập mã màu" style="width:350px" /></div>
                                @php
                                    $rgb = $color->rgb;
                                    $rgb = str_replace(' ', ',', $rgb)
                                @endphp
